...
ALTER TABLE `cars` ADD UNIQUE (`car_license_plate`);
...
ALTER TABLE `subscriptions` ADD FOREIGN KEY (`car_id`) REFERENCES `cars` (`car_id`);
ALTER TABLE `subscriptions` ADD FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`);

ALTER TABLE `user_credits` ADD FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`);
ALTER TABLE `user_credits` ADD FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`);
...
ALTER TABLE `wash_log` ADD FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`);
